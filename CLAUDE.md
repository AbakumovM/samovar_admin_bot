...
`GET /internal/users/{telegram_id}/payment-status`). Эти два критерия ничего не решают
самостоятельно — они только повышают счётчик и открывают дополнительные действия:
при `criteria_matched >= 2` в дайджесте антифрода появляется кнопка «Заблокировать»
(требует известного `telegram_id` пользователя), позволяющая администратору вручную
заблокировать подписку — вызывает `POST /internal/users/{telegram_id}/block` на
samovarbot и переводит подписку в `DISABLED` в панели. При `criteria_matched == 3` и
включённом `ANTIFRAUD_AUTO_BLOCK_ENABLED=true` блокировка выполняется автоматически без
кнопки, уведомление приходит как батч-дайджест авто-блоков. По умолчанию авто-блок
выключен (`ANTIFRAUD_AUTO_BLOCK_ENABLED=false`), кнопка и дайджесты работают независимо.
