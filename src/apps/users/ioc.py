...
from src.apps.users.application.interactor import UserTrafficInteractor
...
class UserTrafficInteractorsProvider(Provider):
    scope = Scope.REQUEST

    @provide
    async def user_traffic_interactor(
        self, gateway: UserTrafficGateway
    ) -> UserTrafficInteractor:
        return UserTrafficInteractor(gateway=gateway)
